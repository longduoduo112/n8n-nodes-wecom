...
export const listUserIdsDescription: INodeProperties[] = [
	{
		...
		placeholder: 'xxxxxxx',
		description: '可选。用于分页查询的游标，字符串类型，由上一次调用返回，首次调用不填。如果该字段返回空则表示已没有更多数据。首次调用不填，后续调用填上次返回的 next_cursor。<a href="https://developer.work.weixin.qq.com/document/path/96067" target="_blank">官方文档</a>',
	},
	{
		displayName: '每页数量',
		name: 'limit',
